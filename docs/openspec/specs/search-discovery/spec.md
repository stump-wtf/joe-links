...
> **Known asymmetry (recorded, accepted — PR #281).** Server-side keyword matching is *not* case-folded: the keyword store is queried with the raw host or first path segment, exactly as before this spec. When a keyword and a link share a name (e.g. keyword `gh` and link `gh`), `/gh/x` routes via the keyword while `/GH/x` falls through keyword routing and resolves via the case-folded slug link — the same path shape reaching a different destination depending on case. This is faithful to the precedence freeze above (this spec explicitly leaves keyword behavior untouched); case-folding keyword matching is SPEC-0008 / ADR-0011 territory for a future revision.
...
Visibility filtering MUST be enforced in the shared store layer (never in the handler): non-admin callers MUST receive only links they own or co-own, links shared with the viewer via `link_shares`, and `public` links; other users' `private` and `secure` links MUST NOT appear. Admin callers receive all links (SPEC-0010 REQ "Admin Visibility Override"). In addition to the visibility filter, the candidate set MUST exclude expired and archived links for all callers (SPEC-0020) — suggesting a link that will not resolve is worse than no suggestion. Matching MUST be performed only against fields the caller is authorized to read under SPEC-0010, so that a future field addition cannot turn matching into a content-probing oracle for data the caller could not read directly.
