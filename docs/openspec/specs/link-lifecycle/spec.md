...
> **Portability caveat (recorded — PR #283, migration `00016`).** The lifecycle columns use plain `TIMESTAMP`, the only type all three drivers accept as portable SQL (postgres has no `DATETIME`; matches the existing `api_tokens.expires_at` precedent). MySQL's `TIMESTAMP` range ends at `2038-01-19T03:14:07Z`, so a far-future expiry that sqlite3 and postgres accept surfaces as a strict-mode error (HTTP 500) on MySQL; no validation-layer upper bound is enforced today. If uniform cross-driver behavior is wanted, a future revision should add a validation upper bound (reject `expires_at` beyond `2038-01-19T03:14:07Z`) so MySQL's driver error becomes a clean validation failure everywhere.
...
> **Surface scope (recorded — PR #290).** The one-click renew button renders on the dashboard and tag-detail rows (the capability-carrying `RowCaps` surfaces named by the scenarios below). The admin links table shows the expired badge *without* a renew button: its rows have a different shape with inline editing, through which an admin can already clear an expiration. Revisit only if a dedicated admin renew affordance is ever specified.
...
**Politeness.** The checker MUST NOT hammer: at most 4 probes in flight at once, at least 1 second between successive probes to the same host, and per-link exponential backoff on failure — after `n` consecutive failures the next check MUST be scheduled no sooner than `interval × 2^(n−1)`, capped at `7 × interval`. Healthy links are checked once per interval: the interval governs the **per-link cadence** via `next_check_at`, not the scheduler tick. The checker's wake-up loop MAY (and does — PR #292) poll more often than the interval (`interval/8`, capped at 5 minutes), probing only links that are due, so a coarse tick cannot stretch the effective per-link cadence toward twice the interval. A link is **broken** when `consecutive_failures >= 3`; the broken state MUST be derived from the failure counter, not stored separately.
...
**Serialization convention (settled — PRs #283/#290/#292).** Parity of the lifecycle and health fields holds *semantically* (same fields, same values, same capability gating), not byte-for-byte. REST responses serialize the nullable lifecycle timestamps as explicit nulls (`"expires_at": null`, `"archived_at": null`) and omit `health`/`health_checks_disabled` for callers without capabilities; MCP payloads use `omitempty` throughout — an absent `expires_at`/`archived_at` means null ("never expires" / "not archived"), and an absent `health`/`health_checks_disabled` means the caller holds no capabilities on the link. On input, MCP `update_link` distinguishes an explicit JSON `null` for `expires_at` (clears it) from an omitted field (unchanged), matching REST `PUT` tri-state semantics; MCP additionally accepts an empty string as a clear.
